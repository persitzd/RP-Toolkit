...
num_of_columns = (1 + 2*GARP_flags(3) + 2*GARP_flags(4)) * (GARP_flags(5) + 2*GARP_flags(6) + 2*GARP_flags(7));
...
if (GARP_flags(7) == 1 && SARP_full ~= 0)
    ...
    % full value
    WARP_GARP_SARP_Mat(:, col_counter) = SARP_full_pairs;
    % update the column
    col_counter = col_counter + 1;
    ...
    if GARP_flags(3)
        ...
        % this loop finds the residual as (%) of the total SARP
        for i=1:obs_num
            WARP_GARP_SARP_Mat(i, col_counter+1) = WARP_GARP_SARP_Mat(i, col_counter) / SARP_full_pairs;
        end
    ...
    if GARP_flags(4)
        for i=1:obs_num
            WARP_GARP_SARP_Mat(i, col_counter) = Mat_GARP(i, 9);
            WARP_GARP_SARP_Mat(i, col_counter+1) = Mat_GARP(i, 10);
        end
