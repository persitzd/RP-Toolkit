...
elseif action_flag == HPZ_Constants.MMI_action   % MMI
    file_val_str{aggregation_flag + 3} = 'Criterion';
elseif action_flag == HPZ_Constants.BI_action   % BI
    file_val_str{7} = 'Criterion';
end
...
if action_flag == HPZ_Constants.NLLS_action
    ...
    elseif metric_flag == HPZ_Constants.normalized_euclidean_metric     % NLLS - normalized-Euclidean metric 
...
figure_title = char(strcat('Output File Format (', HPZ_Constants.current_run_screen, {' '}, num2str(runs_counter), ')'));
...
function [] = ok_button_call(varargin)
    ...
    if strcmp(get(S.file_setting_op(1), 'enable'), 'off') == true
        output_file_config_save(2:7) = output_file_config(2:7);
    elseif strcmp(get(S.file_setting_op(2), 'enable'), 'off') == true
        output_file_config_save([1,3:7]) = output_file_config([1,3:7]);
    elseif strcmp(get(S.file_setting_op(3), 'enable'), 'off') == true
        output_file_config_save([1:2,4:7]) = output_file_config([1:2,4:7]);
    elseif strcmp(get(S.file_setting_op(4), 'enable'), 'off') == true
        output_file_config_save([1:3,5:7]) = output_file_config([1:3,5:7]);
    elseif strcmp(get(S.file_setting_op(5), 'enable'), 'off') == true
        output_file_config_save([1:4,6:7]) = output_file_config([1:4,6:7]);
    elseif strcmp(get(S.file_setting_op(6), 'enable'), 'off') == true
        output_file_config_save([1:5,7]) = output_file_config([1:5,7]);
    elseif strcmp(get(S.file_setting_op(7), 'enable'), 'off') == true
        output_file_config_save(1:6) = output_file_config(1:6);
    end
