function [data_name, choice_set_type, locations, pref_class, ok] = HPZ_Screen_Data_Set_Adding(data_name)
...
%% Type of Choice set (linear budget or a finite set)

% current bottom coordinate (HOUTMAN-MAX is just below VARIAN)
current_bottom = current_bottom - move_down;

% current height of element
current_height = radio_button_height*3;
% current bottom coordinate
current_bottom = current_bottom - current_height;
choice_set_type_group = uibuttongroup('Parent',panel, ...
    'units','pix', ...
    'title', 'Type of Choice set', ...
    'pos',[figure_width/5 , current_bottom , figure_width*3/5 , current_height]);

choice_set_type_rd(1) = uicontrol(choice_set_type_group, ...
    'value', 1, ...
    'enable','on', ...
    'style','rad', ...
    'unit','pix', ...
    'position',[10 , radio_button_height*1.2 , figure_width*3/5-10 , radio_button_height], ...
    'string','Choice from Budget Line (assuming monotonicty)', ...
    'callback',{@check_button_call});

choice_set_type_rd(2) = uicontrol(choice_set_type_group, ...
    'value', 0, ...
    'enable','on', ...
    'style','rad', ...
    'unit','pix', ...
    'position',[10 , radio_button_height*0.2 , figure_width*3/5-10 , radio_button_height], ...
    'string','Choice from a Finite Set (assuming monotonicty)', ...
    'callback',{@check_button_call});
...
% current bottom coordinate
current_bottom = current_bottom - current_height;
pref_class_group = uibuttongroup('Parent',panel, ...
    'units','pix', ...
...
pref_class_rd(1) = uicontrol(pref_class_group, ...
    'value', 1, ...
    'enable','on', ...
    'style','rad', ...
    'unit','pix', ...
...
pref_class_rd(2) = uicontrol(pref_class_group, ...
    'value', 0, ...
    'enable','on', ...
    'style','rad', ...
    'unit','pix', ...
...
pref_class_rd(3) = uicontrol(pref_class_group, ...
    'value', 0, ...
    'enable','on', ...
    'style','rad', ...
    'unit','pix', ...
...
pref_class_rd(4) = uicontrol(pref_class_group, ...
    'value', 0, ...
    'enable','on', ...
    'style','rad', ...
    'unit','pix', ...
...
pref_class_rd_OR_label = uicontrol(pref_class_group, ...
    'style','text', ...
    'unit','pix', ...
...
function [] = check_button_call(varargin)
    ...
    % choice set type
    if get(choice_set_type_rd(1), 'value') == 1.0
        choice_set_type = HPZ_Constants.choice_set_budget_line;
        % then the user must specify the columns
        for element_number = 1:length(input_elements_list) 
            element = input_elements_list{element_number};
            set(element, 'enable', 'on');
        end
    elseif get(choice_set_type_rd(2), 'value') == 1.0
        choice_set_type = HPZ_Constants.choice_set_finite_set;
        % user doesn't need and can't specify columns when using this option   
        for element_number = 1:length(input_elements_list) 
            element = input_elements_list{element_number};
    ...
    if isempty(data_name) || isinf(pref_class) || choice_set_type == 0 || ...
            (choice_set_type == HPZ_Constants.choice_set_budget_line) && ...
            ( length(locations) < 6 || length(unique(locations_full)) < length(locations_full) || ...
