function [numeric_flag, function_flag, param1_restrictions, param2_restrictions, fix_corners, ok] = HPZ_Screen_Functional_Form_Settings_Risk(action_flag, choice_set_type, main_folder, runs_counter)
...
[numeric_flag, function_flag, param1_restrictions, param2_restrictions, fix_corners] = HPZ_Functional_Form_risk_Settings_Read(main_folder);
...
if choice_from_finite_set_flag
    numeric_flag = HPZ_Constants.analytic;   % in the case of choice from finite set, we always use analytic calculations
    fix_corners = 0;
end
...
S.solution_option_rd(1) = uicontrol(S.bg_na,...
    ...
    'enable',enable{2-choice_from_finite_set_flag},...
    'style','rad',...
    'unit','pix',...
    'fontsize',font_size,...
...
S.solution_option_rd(3) = uicontrol(S.bg_na,...
    ...
    'enable', enable{1 + 1*(~choice_from_finite_set_flag && (action_flag == HPZ_Constants.MMI_action || action_flag == HPZ_Constants.BI_action))},...
    'style','rad',...
    'unit','pix',...
    'fontsize',font_size,...
...
function [] = ok_button_call(varargin)
    ...
    HPZ_Functional_Form_risk_Settings_Write(numeric_flag, function_flag, param1_restrictions, param2_restrictions, fix_corners, main_folder);
